...
from elric.core.utils import datetime_ceil, convert_to_datetime, datetime_repr, astimezone
from elric.trigger.base import BaseTrigger
from elric.trigger.cron.fields import BaseField, WeekField, DayOfMonthField, DayOfWeekField, DEFAULT_VALUES
...
class CronTrigger(BaseTrigger):
    ...
    def get_next_trigger_time(self, previous_fire_time, now=None):
        ...
        if previous_fire_time:
            start_date = max(curr_time, previous_fire_time + timedelta(microseconds=1))
        else:
            start_date = max(curr_time, self.start_date) if self.start_date else curr_time
